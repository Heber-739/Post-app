...
export class CommentsComponent implements OnInit, OnDestroy {
  ...
  private getComments(): void {
    this.commentsService.getCommentsByPostId(this.post.id).then((res) => {
  ...
  sendEditComment(comment: Comment | null): void {
    this.editComment = comment;
  }

  deleteComment(id: string): void {
    this.commentsService.deleteComment(id, this.post.id).then((res) => {
      if (res) {
  ...
  viewLikes(data: Author[] | null): void {
    this.likes = data;
  }
  ...
  saveComment(update: string): void {
    if (!this.post) return;
    if (this.editComment && !this.addComment) {
      this.editComment.comment = update;
      this.commentsService.updateComment(this.editComment).then(() => {
  ...
  addNewComment(comment: string): void {
    ...
    this.commentsService.createComment(create).then((res) => {
